...
func TestSubmitsToDeniedUrl(t *testing.T) {
	...
	for _, url := range helper.mockURLSdenied {
		...
		time.Sleep(3 * time.Second) // Added because with async worker the test was checking for fail/success values before worker could actually try sending the request.
		fmt.Print("\n***************")
